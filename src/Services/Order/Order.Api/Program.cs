...
builder.Services.AddHealthChecks()
    .AddDbContextCheck<OrderDbContext>(tags: [ServiceHealthCheckTags.Readiness])
    .AddCheck<OrderReferenceDataHealthCheck>(
        "order-reference-data",
        tags: [ServiceHealthCheckTags.Readiness]);
builder.Services.AddSingleton(TimeProvider.System);
builder.Services.AddOptions<OrderWorkflowOptions>()
    .Bind(builder.Configuration.GetSection(OrderWorkflowOptions.SectionName))
    .Validate(
        options => options.CheckoutTimeout > TimeSpan.Zero,
        "CheckoutTimeout must be positive.")
    .ValidateOnStart();
...
builder.Services.AddRabbitMqWithPostgresOutbox<OrderDbContext>(
    ...
    configureRegistrations: registration =>
    {
        ...
        registration.AddConsumer<CustomerIdentitySnapshotPageConsumer>();
        registration.AddConsumer<ProductCatalogChangedConsumer>();
        registration.AddConsumer<ProductCatalogSnapshotPageConsumer>();
        registration.AddConsumer<InventoryReservedConsumer>();
        registration.AddConsumer<InventoryRejectedConsumer>();
        registration.AddConsumer<PaymentActionRequiredConsumer>();
        registration.AddConsumer<PaymentAuthorizedConsumer>();
        registration.AddConsumer<PaymentRejectedConsumer>();
        registration.AddConsumer<InventoryReservationCommittedConsumer>();
...
builder.Services.AddIntegrationCommandRoute<SynchronizeProductCatalogSnapshot>(
    SynchronizeProductCatalogSnapshot.EndpointName);
...
builder.Services.AddIntegrationCommandRoute<CommitInventoryReservation>(
    CommitInventoryReservation.EndpointName);
builder.Services.AddIntegrationCommandRoute<AuthorizeOrderPayment>(
    AuthorizeOrderPayment.EndpointName);
builder.Services.AddIntegrationCommandRoute<CaptureOrderPayment>(
    CaptureOrderPayment.EndpointName);
builder.Services.AddIntegrationCommandRoute<CancelOrderPayment>(
