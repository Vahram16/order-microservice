...
public sealed class OrderItem
{
    ...
    internal static Result<OrderItem> Create(Guid orderId, OrderItemDraft draft)
    {
        ...
        if (orderId == Guid.Empty || draft.ProductId == Guid.Empty || draft.Quantity <= 0 ||
            string.IsNullOrWhiteSpace(sku) || sku.Length > 64 ||
            string.IsNullOrWhiteSpace(name) || name.Length > 200 ||
            draft.UnitPrice < 0m || decimal.Round(draft.UnitPrice, 2) != draft.UnitPrice ||
            draft.UnitPrice > decimal.MaxValue / draft.Quantity)
        {
