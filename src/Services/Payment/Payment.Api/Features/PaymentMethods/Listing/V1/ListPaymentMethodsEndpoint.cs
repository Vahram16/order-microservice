...
internal static class ListPaymentMethodsEndpoint
{
    ...
    .WithName("ListPaymentMethods").WithSummary("Lists the authenticated customer's saved payment methods.").RequireAuthorization(RolePolicy.For(PaymentAuthorization.Role), ScopePolicy.For(PaymentAuthorization.ReadScope)).Produces<IReadOnlyList<PaymentMethodResponse>>(200).ProducesProblem(401).ProducesProblem(403);
}
