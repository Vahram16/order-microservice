...
internal static class SetDefaultPaymentMethodEndpoint
{
    ...
    .WithName("SetDefaultPaymentMethod").WithSummary("Sets one saved payment method as the authenticated customer's default.").RequireAuthorization(RolePolicy.For(PaymentAuthorization.Role), ScopePolicy.For(PaymentAuthorization.WriteScope)).Produces<PaymentMethodResponse>(200).ProducesProblem(401).ProducesProblem(403).ProducesProblem(404).ProducesProblem(409);
}
