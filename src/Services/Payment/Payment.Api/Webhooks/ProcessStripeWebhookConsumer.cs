    ...
    : IConsumer<ProcessStripeWebhook>
{
    ...
    private async Task ProcessOrderPaymentAsync(PaymentWebhookEvent webhookEvent, CancellationToken cancellationToken)
    {
        ...
        if (attempt.Status is OrderPaymentStatus.Cancelled or OrderPaymentStatus.Rejected or OrderPaymentStatus.CaptureFailed)
        {
            if (string.Equals(session.Status, "requires_capture", StringComparison.Ordinal))
                session = await orderPaymentProvider.CancelAsync(session.ProviderPaymentIntentId, OrderPaymentProviderIdempotencyKeys.Cancel(attempt.OrderId), cancellationToken);
            ...
            switch (session.Status)
            {
                ...
                case "succeeded":
                    if (attempt.Status is not (OrderPaymentStatus.Authorized or OrderPaymentStatus.Captured))
                        throw WebhookProcessingException.Permanent("payment.webhook.unexpected_captured_payment_requires_manual_reconciliation");
