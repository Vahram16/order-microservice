...
builder.AddApiDocumentation("Customer API", new ApiDocumentationOAuthOptions("mobile-app", "https://localhost:7050/scalar/v1", new Dictionary<string, string>(StringComparer.Ordinal)
{
    ["openid"] = "Authenticate the user.", ["profile"] = "Read the user's basic identity profile.", ["email"] = "Read the user's verified email address.", ["backend-api-audience"] = "Request a token for the backend API."
}));
...
builder.Services.AddHealthChecks().AddDbContextCheck<CustomerDbContext>(tags: [ServiceHealthCheckTags.Readiness]);
...
builder.Services.AddRabbitMqWithPostgresOutbox<CustomerDbContext>(builder.Configuration, "customer", configureRegistrations: registration => registration.AddConsumer<CustomerIdentitySnapshotConsumer>());
builder.Services.AddIntegrationCommandRoute<SynchronizeCustomerIdentitySnapshot>(SynchronizeCustomerIdentitySnapshot.EndpointName);
...
app.MapDefaultEndpoints(); app.MapApiDocumentation(); app.MapMicroserviceErrorCatalog(); CustomerErrorCatalog.Map(app); app.MapCustomerEndpoints();
